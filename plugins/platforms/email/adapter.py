...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        try:
            ...
            message_id = await loop.run_in_executor(
                None, self._send_email, chat_id, content, reply_to, thread_id, explicit_subject
            )
    ...
    def _send_email(
        ...
        # last dispatch's subject). The plain-sender key is consulted ONLY for
        # genuine replies (a reply-to message id is present): bare outbound
        # sends (cron deliveries) must never inherit the sender's last inbound
    ...
    def _send_email_with_attachments(
        ...
        # thread_id → compound chat_id) so attachment replies carry the
        # right thread's subject. Bare outbound sends with an explicit
        # subject (cron deliveries) get a verbatim subject and a fresh
    ...
    async def send_document(
        ...
        **kwargs,
        ...
        try:
            ...
            explicit_subject = (kwargs or {}).get("metadata", {}).get("subject")
            message_id = await loop.run_in_executor(
                None,
